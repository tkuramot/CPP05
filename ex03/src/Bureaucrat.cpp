...
void Bureaucrat::ExecuteForm(Form const &form) {
  try {
    ...
    std::cout << kName << "'s grade was too low to execute " << form.GetName()
              << "." << std::endl;
  } catch (Form::NotSignedYet &e) {
    std::cout << kName << " tried to execute " << form.GetName()
              << ", but it was not signed yet." << std::endl;
  } catch (std::exception &e) {
    std::cout << kName << " couldn't execute " << form.GetName() << "."
              << std::endl
              << e.what() << std::endl;
  }
}
